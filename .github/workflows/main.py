...
def compile_tex_to_pdf(
    ...
    for tex_file in tex_files:
        ...
        try:
            ...
            if pdf_path.exists():
                relative_parts = (
                    tex_dir.relative_to(src_dir).parts if tex_dir != src_dir else ()
                )
...
@maybe_beartype
def build_tree(
    path: Path, depth: int = 0, max_depth: Optional[int] = MAX_DEPTH
) -> Dict[str, Any]:
    node: Dict[str, Any] = {}
...
def generate_html(node: Dict[str, Any], level: int = 2, indent: int = 0) -> str:
    ...
    sorted_keys = sorted(
        node.keys(),
        key=lambda k: (
            SECTION_ORDER.index(k) if k in SECTION_ORDER else 1000,
            k.lower(),
        ),
    )
    for key in sorted_keys:
        if key == "_files":
            for name, path in node["_files"]:
                ...
                tag = f"h{min(level, 4)}"
                html_lines.append(
                    f'{space}<{tag}><a href="./{rel}" target="_blank">{name}</a></{tag}>'
                )
        else:
            tag = f"h{min(level, 4)}"
            if level == 2:
...
if __name__ == "__main__":
    ...
    # file_path = os.path.join(base_dir, "..", "..", "Documentazione", "test.txt")
